...
## Müşteri profili ve paket kimlikleri

Build `-PCUSTOMER=<profil>` ile seçilir; varsayılan profil `phosfe`'dir. Uygulama adı,
applicationId, APK öneki ve sürüm `customers/<profil>/profile.properties` içinden gelir. Debug ve demo
paketlerine sırasıyla `.debug` ve `.demo` suffix eklenir; release profilin applicationId değerini
aynen kullanır.

Standalone/ECR ve Datecs/Newland/Simulator ayrımı flavor boyutlarıyla yapılır. APK dosya adı
`<ARTIFACT_NAME>-<MODE>-<DEVICE>-<BUILD>-v<VERSION>.apk` biçimindedir.

## İmzalama

`customers/<profil>/keystore.properties.example` dosyası aynı dizinde `keystore.properties` adıyla
kopyalanıp müşterinin release keystore bilgileri girilir. Gerçek property ve JKS/keystore dosyaları
...
`<applicationId>.database.wrapper.v1` anahtarıyla AES-GCM olarak sarılıp uygulama özel alanında tutulur.
ISO mesajları ve tam PAN ayrıca alan seviyesinde AES-GCM zarfıyla korunur.

## BKM ortamı

`customers/<profil>/bkm-environment.properties.example`, test ve production primary/secondary host, port, vendor ID,
...
`TECHPOS_<ALAN_ADI>` environment variable veya Gradle `-P<ALAN_ADI>=...` parametresiyle verilebilir.
